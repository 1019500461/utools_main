  ...
  <section>
    <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
      <div>
        ...
        <p class="mt-1 text-sm text-slate-500">管理角色信息，并设置菜单和接口权限。</p>
      </div>
    ...
    <n-card :bordered="false" class="mb-4">
      <div class="flex flex-wrap items-center gap-3">
        <n-input v-model:value="query.role_name" clearable class="max-w-xs" placeholder="请输入角色名称" @keyup.enter="loadRoles" />
    ...
    <n-drawer v-model:show="authDrawerVisible" :width="520" placement="right">
      <n-drawer-content title="设置权限">
        <div class="mb-4 flex items-center gap-2">
          <n-input v-model:value="treePattern" clearable placeholder="筛选权限" />
          <n-button type="primary" :loading="savingAuth" @click="saveAuthorized">保存</n-button>
        </div>
        <n-tabs type="line">
    ...
    render(row) {
      return h(NSpace, null, {
        default: () => [
          ...
          h(NButton, { size: 'small', type: 'info', secondary: true, onClick: () => openAuthorized(row) }, { default: () => '设置权限' }),
        ],
      })
    },
  },
...
async function saveRole() {
  ...
  try {
    if (modalMode.value === 'create') {
      await api.createRole({ name: form.name, desc: form.desc })
      message.success('创建成功')
    } else {
      await api.updateRole({ id: form.id, name: form.name, desc: form.desc })
